...
function createNewArray(noOfBars = 60) {
    ...
    const containerHeight = 500; // px, as set in CSS
    const maxBarHeight = containerHeight * 0.9; // 90% of container
    ...
    for (let i = 0; i < noOfBars; i++) {
        ...
        // Normalize height
        const normalizedHeight = (array[i] / maxVal) * maxBarHeight;
        bar.style.height = `${normalizedHeight}px`;
        bar.classList.add('bar');
        bar.classList.add('flex-item');
        bar.classList.add(`barNo${i}`);
        
        // Add the number value as a label
